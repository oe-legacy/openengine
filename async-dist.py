...
class Async_repo_download(Thread):
   def __init__ (self, cmd, r, p):
...
def run_repo(repos):
    ...
    for p,r in repos:
        ...
        current = Async_repo_download(cmd, r, p)
...
class Async_file_download_and_unpack(Thread):
   def __init__ (self, res, file, dir):
        ...
        self.dir = dir
   def run(self):
        f, h = urllib.urlretrieve(self.res, self.file)
        unpack(h.gettype(), self.file, self.dir)
...
def run_data(data):
    ...
    for dir, res in data:
        ...
        current = Async_file_download_and_unpack(res, file, dir)
        threadlist.append(current)
        current.start()
    for thread in threadlist:
        thread.join()
